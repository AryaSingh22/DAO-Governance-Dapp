...
contract Treasury is Ownable, Pausable, ReentrancyGuard {
    ...
    event ResearchFunded(address indexed recipient, uint256 amount, uint256 duration, uint256 startTime);
    ...
    // Streaming payments
    function createStream(address recipient, uint256 amount, uint256 duration) external nonReentrant {
        _createStream(recipient, amount, duration);
        emit StreamCreated(recipient, amount, duration, block.timestamp);
    }


    // Alias for createStream to match interface requirements and checklist
    function fundResearch(address recipient, uint256 amount, uint256 duration) external nonReentrant {
        _createStream(recipient, amount, duration);
        emit ResearchFunded(recipient, amount, duration, block.timestamp);
    }

    function _createStream(address recipient, uint256 amount, uint256 duration) internal onlyOwner whenNotPaused {
